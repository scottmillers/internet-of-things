...
TSL2561.prototype.readRegister = function (register, callback) {
    ...
    self.wire.readByte(self.options.address, readCmd, function (err, bytes) {
        callback(err, bytes);
    });
};
...
TSL2561.prototype.setPowerMode = function (newMode, callback) {
    ...
    async.waterfall([
        ...
        function (oldReg, cB) {
            ...
            self.wire.writeByte(self.options.address, writeCmd, writeVal, function (err) {
                if (err) {
...
TSL2561.prototype.getLight0 = function (callback) {
    ...
    self.wire.readWord(self.options.address, readLightCmd, function (err, bytes) {
        if (err) {
            if (callback) callback(new Error('read on channel 0 failure'), null);
            return;
        }
        li = ((bytes & 0xff) << 1) + (bytes >> 15);
